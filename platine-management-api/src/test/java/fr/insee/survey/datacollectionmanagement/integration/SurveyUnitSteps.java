...
public class SurveyUnitSteps {
    ...
    public void iFindTheSurveyUnitDetailWithIdSu(String expectedIdSU) throws Exception {
        ...
        SurveyUnitDetailsDto contentDto = jsonMapper.readValue(content, SurveyUnitDetailsDto.class);
        Assertions.assertThat(contentDto.getIdSu()).isEqualTo(expectedIdSU);
    }

    @Then("I find the surveyUnitDetail with identificationName {string}")
    public void iFindTheSurveyUnitDetailWithIdentificationName(String expectedIdentificationName)
            throws UnsupportedEncodingException, JacksonException {
        MvcResult mvcResult = result.andReturn();
        String content = mvcResult.getResponse().getContentAsString();
        SurveyUnitDetailsDto contentDto = jsonMapper.readValue(content, SurveyUnitDetailsDto.class);
        Assertions.assertThat(contentDto.getIdentificationName()).isEqualTo(expectedIdentificationName);
    }

    @When("I get survey unit details by idSu {string}")
    public void iGetSurveyUnitDetailsByIdSu(String idSu) throws Exception {
        result = mockMvc.perform(
                get(UrlConstants.API_SURVEY_UNITS_ID, idSu)
