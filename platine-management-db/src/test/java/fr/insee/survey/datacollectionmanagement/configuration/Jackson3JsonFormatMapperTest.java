...
class Jackson3JsonFormatMapperTest {
    ...
    @Test
    @DisplayName("readFromSource should deserialize LocalDateTime when the appropriate module is registered")
    void readFromSource_shouldHandleLocalDateTime_ifModulePresent() throws Exception {
        JsonMapper jsonMapper = JsonMapper.builderWithJackson2Defaults().build();
        Jackson3JsonFormatMapper mapper = new Jackson3JsonFormatMapper(jsonMapper);

        @SuppressWarnings("unchecked")
        JavaType<LocalDateTime> javaType = Mockito.mock(JavaType.class);
        WrapperOptions options = Mockito.mock(WrapperOptions.class);

        when(javaType.getJavaType()).thenReturn(LocalDateTime.class);

        JsonParser parser = jsonMapper.createParser("\"2026-01-22T13:46:26\"");

        // This can fail if JavaTimeModule is not available/registered in the shaded tools.jackson setup.
        // We document both possible outcomes.
        try {
            LocalDateTime value = mapper.readFromSource(javaType, parser, options);
            assertThat(value).isEqualTo(LocalDateTime.parse("2026-01-22T13:46:26"));
